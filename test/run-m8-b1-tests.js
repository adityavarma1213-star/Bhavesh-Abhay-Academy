...
async function testSuccessfulEvaluationRecorded() {
  ...
  assert(res.submission.evaluation.imageEvaluated === false, 'B6: result honestly marks imageEvaluated:false (M8-B1 is text-only)');
...
async function testImageAttachedNotedButNotSent() {
  let capturedBody = null;
  const fetchImpl = (url, opts) => {
    capturedBody = JSON.parse(opts.body);
    return jsonResponse(200, {
      schemaVersion: 1, evaluationType: 'text_only', overallAssessment: 'uncertain', summary: 'ok', strengths: [], mistakes: [], suggestions: [],
  ...
  assert(capturedBody.imageAttached === true, 'B9: request body honestly flags imageAttached:true when a photo was attached');
  assert(!('image' in capturedBody) && !('imageData' in capturedBody) && !('dataUrl' in capturedBody), 'B10: request body never includes any image bytes/data field (M8-B1 is text-only)');
  assert(Object.keys(capturedBody).sort().join(',') === 'imageAttached,subjectHint,text', 'B11: request body contains exactly text, subjectHint, imageAttached — nothing else');
}
...
async function main() {
  ...
  await testImageAttachedNotedButNotSent();
