...
async function testScannedPdfHonesty() {
  global.document={createElement:()=>({width:0,height:0,getContext:()=>({}),toDataURL:()=> 'data:image/jpeg;base64,AAAA'})};
  const PDF = freshPdf({
    getDocument: () => ({ promise: Promise.resolve({
      numPages: 1,
      getPage: async () => ({ getTextContent: async () => ({ items: [] }), getViewport:()=>({width:100,height:100}), render:()=>({promise:Promise.resolve()}) })
    }) })
  });
  const result = await PDF.extractText(fakeFile('application/pdf',100,'scan.pdf'));
  assert(result.ok === true && result.scannedImageDataUrls.length === 1,'C-9: image-only/scanned PDF is rendered to a transient image for vision evaluation');
