...
// Student-controlled learning path with bounded local editing and an authenticated server-backed sync path.
(function (global) {
  'use strict';
  ...
  async function loadServer(learnerId) {
    ...
    try { const response = await fetch(`/api/m02-custom-mode?learnerId=${encodeURIComponent(learnerId)}`, { credentials: 'include', cache: 'no-store', headers: { Accept: 'application/json' } }); const data = await response.json().catch(() => null); if (!response.ok) return { ok: false, error: data?.error || { code: 'CUSTOM_MODE_SERVER_READ_FAILED' } }; const path = normalizePath(data?.path); savePath(path); return { ok: true, path, updatedAt: data?.updatedAt || null }; } catch { return { ok: false, error: { code: 'CUSTOM_MODE_NETWORK_ERROR' } }; }
  }
  async function saveServer(learnerId, path = getPath(), expectedUpdatedAt = null) {
    if (!learnerId) return { ok: false, error: { code: 'LEARNER_ID_REQUIRED' } };
    const normalized = normalizePath(path);
    try { const response = await fetch(`/api/m02-custom-mode?learnerId=${encodeURIComponent(learnerId)}`, { method: 'PUT', credentials: 'include', cache: 'no-store', headers: { 'Content-Type': 'application/json', Accept: 'application/json' }, body: JSON.stringify({ ...normalized, expectedUpdatedAt }) }); const data = await response.json().catch(() => null); if (!response.ok) return { ok: false, error: data?.error || { code: 'CUSTOM_MODE_SERVER_WRITE_FAILED' } }; return { ok: true, path: normalizePath(data?.path || normalized), updatedAt: data?.updatedAt || null }; } catch { return { ok: false, error: { code: 'CUSTOM_MODE_NETWORK_ERROR' } }; }
  }
  async function clearServer(learnerId) { if (!learnerId) return { ok: false, error: { code: 'LEARNER_ID_REQUIRED' } }; try { const response = await fetch(`/api/m02-custom-mode?learnerId=${encodeURIComponent(learnerId)}`, { method: 'DELETE', credentials: 'include', cache: 'no-store', headers: { Accept: 'application/json' } }); const data = await response.json().catch(() => null); if (!response.ok) return { ok: false, error: data?.error || { code: 'CUSTOM_MODE_SERVER_DELETE_FAILED' } }; return savePath(normalizePath(data?.path)); } catch { return { ok: false, error: { code: 'CUSTOM_MODE_NETWORK_ERROR' } }; } }
  global.BAACustomMode = { STORAGE_KEY, SCHEMA_VERSION, getPath, savePath, addStep, removeStep, toggleStep, moveStep, clearPath, validateStep, normalizePath, loadServer, saveServer, clearServer };
})(window);