...
(function (global) {
  ...
  const STATE_KEY = 'custom_mode_v1';
  ...
  function pushSync(path) {
    ...
    fetch(url, opts).catch((e) => {
      ...
      console.warn('[BAA Module 2] Custom Mode sync queued offline', e);
    });
  }
  async function hydrateFromServer(learnerId) {
    ...
    try {
      ...
      if (state) {
        const storage = getStorage();
        if (storage) storage.setItem(STORAGE_KEY, JSON.stringify(normalizePath(state)));
      }
      setSyncTarget(learnerId);
      return true;
    } catch (e) {
      console.warn('[BAA Module 2] Could not hydrate Custom Mode from server — continuing with local data only.', e);
      return false;
    }
  }
