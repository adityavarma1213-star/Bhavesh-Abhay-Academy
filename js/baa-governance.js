...
function entropyToken(){
  try{
    if(global.crypto?.randomUUID) return global.crypto.randomUUID().replace(/-/g,'').slice(0,12);
    if(global.crypto?.getRandomValues){
      const bytes=new Uint8Array(8);global.crypto.getRandomValues(bytes);
...
function create(action){
  ...
  const suffix=entropy?`_${entropy}`:'';
  return {ok:true,error:null,item:{id:`gov_${now}_${sequence}${suffix}`,type,status:'pending_human_review',createdAt:new Date(now).toISOString(),decision:null}};
}
