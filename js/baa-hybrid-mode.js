...
(function (global) {
  ...
  // Backed by the generic /api/v1/client-state endpoint; localStorage
  // remains the immediate, offline-first source of truth.
  // ------------------------------------------------------------
  let syncLearnerId = null;
  const STATE_KEY = 'hybrid_mode_v1';
  function setSyncTarget(learnerId) { syncLearnerId = learnerId || null; }
  function pushSync(path) {
    ...
    const opts = { method: 'PUT', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(path) };
    fetch(url, opts).catch((e) => {
      if (global.BAAOfflineSync) global.BAAOfflineSync.enqueue(url, opts);
      console.warn('[BAA Module 3] Hybrid Mode sync queued offline', e);
    });
  }
  async function hydrateFromServer(learnerId) {
    ...
    try {
      ...
      if (state && global.localStorage) {
        global.localStorage.setItem(STORAGE_KEY, JSON.stringify(normalizePath(state)));
      }
      setSyncTarget(learnerId);
      return true;
    } catch (e) {
      console.warn('[BAA Module 3] Could not hydrate Hybrid Mode from server — continuing with local data only.', e);
      return false;
    }
  }
