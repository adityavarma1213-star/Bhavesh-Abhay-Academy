...
(function(global){
  ...
  if(!C) return;
  let mounted=false, features=[], role=null, roleResolved=false;
  function esc(value){return String(value==null?'':value).replace(/[&<>\"']/g,function(ch){return {'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'}[ch];});}
  function detectRole(){
    role=null; roleResolved=false;
    return fetch('/api/auth/me',{credentials:'include',cache:'no-store',headers:{Accept:'application/json'}}).then(function(r){
      ...
      if(Array.isArray(rs)) role=rs[0]||null; else if(rs) role=rs;
      roleResolved=true;
      return role;
  ...
  function allowed(f){
    ...
    return !!role && (f.roles.includes(role)||f.roles.includes('admin')&&role==='admin');
  }
  ...
  function mount(){
    ...
    function render(){
      ...
      status.textContent=role?'Showing features available to your '+role+' workspace.':'Showing public BAA feature guidance. Sign in for role-specific guidance.';
  ...
  global.BAAGuideRobot={init:init,open:function(){const b=document.getElementById('baaGuideRobotButton');if(b)b.click();},getCurrentRole:function(){return roleResolved?role:null;},getFeatures:function(){return C.getFeatures();}};
  init();
})(window);