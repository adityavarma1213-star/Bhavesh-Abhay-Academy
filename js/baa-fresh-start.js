...
async function deleteServerData(learnerId,confirm){
  const response=await fetch('/api/m55-data-trust.js',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'delete',learnerId:learnerId||undefined,confirm:confirm===true})});
  const data=await response.json().catch(()=>({ok:false,error:{code:'INVALID_RESPONSE',message:'Invalid server response.'}}));
