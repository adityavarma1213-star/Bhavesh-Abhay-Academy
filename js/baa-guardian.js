...
(function(global){
  ...
  async function readJsonResponse(response,fallback){
    ...
    if(Number.isFinite(contentLength)&&contentLength>MAX_RESPONSE_BYTES)return fallback;
    if(response.body&&typeof response.body.getReader==='function'){
      ...
      try{
        while(true){
          ...
          if(total>MAX_RESPONSE_BYTES){await reader.cancel();return fallback;}
    ...
    try{
      ...
      if(new TextEncoder().encode(text).byteLength>MAX_RESPONSE_BYTES)return fallback;
  ...
  function save(store){try{localStorage.setItem(STORAGE_KEY,JSON.stringify(store));return true;}catch{return false;}}
  ...
  function getAcademicAlerts(){
    ...
    const grouped={};
    (store.evidence||[]).forEach(row=>{if(!row||!row.concept)return;(grouped[row.concept]||=[]).push(row);});
    Object.entries(grouped).forEach(([concept,rows])=>{
      const recent=rows.slice(-3);if(recent.length<3)return;
      const scored=recent.filter(r=>r.correctness==='correct'||r.correctness==='incorrect');if(scored.length<3)return;
  ...
  async function getServerSummary(learnerId){
    ...
    try{
      ...
      if(!data)return {status:'unavailable',error:'GUARDIAN_INVALID_RESPONSE',httpStatus:response.status};
      if(!response.ok)return {status:'unavailable',error:data?.error?.code||'GUARDIAN_SERVER_UNAVAILABLE',httpStatus:response.status};
      const acknowledgements=new Set((data.acknowledgements||[]).map(x=>String(x.alertId)).filter(Boolean));
  ...
  function acknowledgeAlert(id){
    if(!id)return false;const store=load();store.acknowledged[String(id)]=new Date().toISOString();store.lastEvaluatedAt=new Date().toISOString();return save(store);
  }
  ...
  async function syncServer(learnerId){
    ...
    const response=await fetch(`/api/m12-guardian?learnerId=${encodeURIComponent(learnerId)}`,{credentials:'include',cache:'no-store',headers:{Accept:'application/json'}}).catch(()=>null);
    if(!response)return {ok:false,error:'GUARDIAN_SERVER_UNAVAILABLE'};
    const data=await readJsonResponse(response,null);
    if(!data)return {ok:false,error:'GUARDIAN_INVALID_RESPONSE',status:response.status};
    if(!response.ok)return {ok:false,error:data?.error?.code||'GUARDIAN_SERVER_UNAVAILABLE',status:response.status};
  ...
  async function acknowledgeAlertServer(learnerId,alertId){
    ...
    const response=await fetch(`/api/m12-guardian?learnerId=${encodeURIComponent(learnerId)}`,{method:'POST',credentials:'include',cache:'no-store',headers:{'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify({alertId:String(alertId)})}).catch(()=>null);
    if(!response)return {ok:false,error:'GUARDIAN_SERVER_UNAVAILABLE'};
    const data=await readJsonResponse(response,{});
    if(!response.ok)return {ok:false,error:data?.error?.code||'GUARDIAN_SERVER_WRITE_FAILED',status:response.status};
    return {ok:true,alertId:String(alertId),acknowledgedAt:data.acknowledgedAt||new Date().toISOString()};
  }

  async function resetAcknowledgementsServer(learnerId){
    if(!learnerId)return {ok:false,error:'LEARNER_ID_REQUIRED'};
    const response=await fetch(`/api/m12-guardian?learnerId=${encodeURIComponent(learnerId)}`,{method:'DELETE',credentials:'include',cache:'no-store',headers:{'Content-Type':'application/json',Accept:'application/json'},body:'{}'}).catch(()=>null);
    if(!response)return {ok:false,error:'GUARDIAN_SERVER_UNAVAILABLE'};
    const data=await readJsonResponse(response,{});
    if(!response.ok)return {ok:false,error:data?.error?.code||'GUARDIAN_SERVER_DELETE_FAILED',status:response.status};
    return {ok:true};
  }
