...
async function generate(facts){
  ...
  const response=await fetch('/api/m57-parent-conversation.js',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});
...
async function list(learnerId){
  const response=await fetch(`/api/m57-parent-conversation.js?learnerId=${encodeURIComponent(learnerId||'')}`);
