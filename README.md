...
**Database portability:** the server uses standard PostgreSQL through a provider-neutral `postgres` driver. Neon, Supabase, self-hosted PostgreSQL, and other compatible PostgreSQL providers are supported at the connection layer; production provider credentials are deployment-specific.
