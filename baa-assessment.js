...
(function (global) {
  ...
  // ---------- G7: real per-learner server sync (Checkpoint 2) ----------
  // Same opt-in, non-blocking pattern as js/baa-planner.js Checkpoint 1.
  // Scope: authenticated sessions sync attempts, answers, results and raw
  // evidence through /api/v1/assessment, while derived learningMemory and
  // mistakePatterns sync through /api/v1/learning-memory. Anonymous/local
  // testing remains local-only by design.
  let syncLearnerId = null;
  let syncInFlight = false;
  let assessmentSyncQueued = false;
  ...
  async function syncNow(){
    if(!syncLearnerId || typeof fetch === 'undefined') return false;
    const store=load();
    const attempts=(store.attempts || []).map(a => ({...a, answers: undefined, questionResults: undefined}));
    const answers=[]; const results=[];
    for(const a of (store.attempts||[])){
      for(const [questionId, rawAnswer] of Object.entries(a.answers||{})) answers.push({id:`${a.id}_${questionId}`,attemptId:a.id,questionId,rawAnswer,answeredAt:a.endTime||a.startTime});
      for(const r of (a.questionResults||[])) results.push({...r,attemptId:a.id,createdAt:a.endTime||a.startTime});
    }
    const rs=await fetch(`/api/v1/assessment?learnerId=${encodeURIComponent(syncLearnerId)}`,{method:'PUT',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({attempts,answers,results,evidence:store.evidence||[]})});
    if(!rs.ok) throw new Error(`server returned ${rs.status}`);
    return true;
  }

  function pushMemorySync(store) {
    ...
    Promise.all([
      fetch(`/api/v1/assessment?learnerId=${encodeURIComponent(syncLearnerId)}`, { method:'PUT', credentials:'include', headers:{'Content-Type':'application/json'}, body:JSON.stringify({attempts,answers,results,evidence:store.evidence||[]}) }),
      fetch(`/api/v1/learning-memory?learnerId=${encodeURIComponent(syncLearnerId)}`, { method:'PUT', credentials:'include', headers:{'Content-Type':'application/json'}, body:JSON.stringify({learningMemory:store.learningMemory,mistakePatterns:store.mistakePatterns}) })
    ]).then(async rs => { for(const r of rs) if(!r.ok) throw new Error(`server returned ${r.status}`); })
      .catch(e => {
        if(global.BAAOfflineSync){
          const learner=encodeURIComponent(syncLearnerId);
          global.BAAOfflineSync.enqueue(`/api/v1/assessment?learnerId=${learner}`,{method:'PUT',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({attempts,answers,results,evidence:store.evidence||[]})});
          global.BAAOfflineSync.enqueue(`/api/v1/learning-memory?learnerId=${learner}`,{method:'PUT',credentials:'include',headers:{'Content-Type':'application/json'},body:JSON.stringify({learningMemory:store.learningMemory,mistakePatterns:store.mistakePatterns})});
        }
        console.warn('[BAA Section B] Server sync queued offline', e);
      })
      .finally(() => { syncInFlight=false; if(syncQueuedAgain){syncQueuedAgain=false;pushMemorySync(load());} });
  }

  // Pulls the learner's server-known mastery data and merges it into the
  // local store (server wins per-concept on conflict, since it's the
  // union of evidence across every device/browser this learner has used —
  // more complete than any single browser's local copy), then arms
  // setSyncTarget so subsequent saves push through.
  async function hydrateFromServer(learnerId) {
    if (!learnerId || typeof fetch === 'undefined') return false;
    try {
      const [memoryRes, assessmentRes] = await Promise.all([
        fetch(`/api/v1/learning-memory?learnerId=${encodeURIComponent(learnerId)}`, { credentials: 'include' }),
        fetch(`/api/v1/assessment?learnerId=${encodeURIComponent(learnerId)}`, { credentials: 'include' })
      ]);
      if (!memoryRes.ok || !assessmentRes.ok) throw new Error(`server returned ${!memoryRes.ok ? memoryRes.status : assessmentRes.status}`);
      const memoryPayload = await memoryRes.json();
      const assessmentPayload = await assessmentRes.json();
      const snapshot = { ...memoryPayload.snapshot, assessmentSnapshot: assessmentPayload.snapshot };
      const store = load();
      if (snapshot.assessmentSnapshot) {
        for (const a of (snapshot.assessmentSnapshot.attempts || [])) {
          if (!store.attempts.some(x => x.id === a.id)) store.attempts.push({ id:a.id, assessmentId:a.assessment_id, assessmentTitle:'', startTime:a.start_time, endTime:a.end_time, answers:{}, questionResults:[], score:a.score, maxScore:a.max_score, status:a.status, evaluationStatus:a.evaluation_status, reviewStatus:a.review_status, attemptNumber:a.attempt_number });
        }
        for (const e of (snapshot.assessmentSnapshot.evidence || [])) if(!store.evidence.some(x=>x.id===e.id)) store.evidence.push({id:e.id,attemptId:e.attempt_id,assessmentId:e.assessment_id,questionId:e.question_id,subject:e.subject,chapter:e.chapter,topic:e.topic,concept:e.concept,difficulty:e.difficulty,correctness:e.correctness,errorType:e.error_type,score:e.score,maxScore:e.max_score,confidence:e.confidence,timestamp:e.created_at});
      }
      for (const concept of Object.keys(snapshot.learningMemory || {})) {
        const server = snapshot.learningMemory[concept];
        const local = store.learningMemory[concept];
        if (!local || (server.evidenceCount || 0) >= (local.evidenceCount || 0)) {
          store.learningMemory[concept] = { ...local, ...server, history: local?.history || [] };
        }
      }
      const byKey = (list) => new Map(list.map(p => [`${p.concept}::${p.errorType}`, p]));
      const mergedPatterns = byKey(store.mistakePatterns);
      (snapshot.mistakePatterns || []).forEach(p => {
        const key = `${p.concept}::${p.errorType}`;
        if (!mergedPatterns.has(key)) mergedPatterns.set(key, { ...p, occurrences: [] });
      });
      store.mistakePatterns = Array.from(mergedPatterns.values());

      localStorage.setItem(STORAGE_KEY, JSON.stringify(store));
      setSyncTarget(learnerId);
      return true;
    } catch (e) {
      console.warn('[BAA Section B] Could not hydrate learning memory from server — continuing with local data only.', e);
      return false;
    }
  }
  ...
  async function gradeWithAI(question, studentAnswer, evalApiUrl, attemptId) {
    ...
    try {
      ...
      return {
        ...
        verdictToken: result.verdictToken || null,
        answered: true,
      };
    } catch (err) {
  ...
  async function submitAttempt(attempt, questions, evalApiUrl, onProgress) {
    ...
    for (const q of questions) {
      ...
      if (isAutoGradable(q)) {
        ...
        result = await gradeWithAI(q, ans, evalApiUrl, attempt.id);
