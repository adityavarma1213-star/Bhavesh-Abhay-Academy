...
export async function currentSession(req) {
  ...
  const result=await sql`SELECT s.id AS session_id,s.user_id,s.expires_at,u.display_name,u.email,
                           ...
                           GROUP BY s.id,u.id`;
  // db.js intentionally normalizes postgres.js to the { rows } shape used
  // by the production API call sites. This was previously checked as
  // result.length, which made every authenticated request appear unauthenticated.
