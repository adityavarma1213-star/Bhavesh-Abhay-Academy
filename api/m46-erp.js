...
async function testConnection(baseUrl, credentialRef) {
  ...
  try {
    ...
    // credentialRef is a deployment-managed reference, not a secret value.
    // Providers that need an actual token should resolve it server-side via
    // their configured adapter; BAA never accepts a browser-supplied secret.
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'test') {
        ...
        await sql`UPDATE erp_connections SET status=${status},last_error=${result.ok ? null : (result.timeout ? 'ERP_PROVIDER_TIMEOUT' : 'ERP_PROVIDER_UNREACHABLE')},updated_at=NOW() WHERE id=${connectionId} AND owner_user_id=${session.user_id}`;
        await writeAudit({ actorUserId: session.user_id, action: 'erp.connection.test', entityType: 'erp_connection', entityId: connectionId, metadata: { provider: connection.rows[0].provider, ok: result.ok, httpStatus: result.httpStatus, timeout: Boolean(result.timeout) } });
        if (!result.ok) return json(res, 502, { ok: false, error: { code: result.timeout ? 'ERP_PROVIDER_TIMEOUT' : 'ERP_PROVIDER_UNREACHABLE', message: result.timeout ? 'ERP provider connection timed out.' : `ERP provider did not accept the connection test${result.httpStatus ? ` (HTTP ${result.httpStatus})` : ''}.` } });
