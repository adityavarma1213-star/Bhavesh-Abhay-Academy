...
function clean(v, max = 200) {
  ...
  if (value.length > max) { const error = new Error(`Value must be at most ${max} characters.`); error.status = 400; error.code = 'VALUE_TOO_LONG'; throw error; }
  return value;
}
...
function body(req) {
  ...
  catch { const error = new Error('Request body must contain valid JSON.'); error.status = 400; error.code = 'INVALID_JSON_BODY'; throw error; }
}
function boundedMetadata(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return {};
  try { const serialized = JSON.stringify(value); const byteLength = typeof TextEncoder === 'function' ? new TextEncoder().encode(serialized).byteLength : Buffer.byteLength(serialized, 'utf8'); if (byteLength > MAX_METADATA_BYTES) return null; return JSON.parse(serialized); } catch { return null; }
}
...
function ipv6ToBigInt(value) {
  const clean = value.split('%')[0].toLowerCase(); const parts = clean.split('::'); if (parts.length > 2) return null;
  const left = parts[0] ? parts[0].split(':') : []; const right = parts.length === 2 && parts[1] ? parts[1].split(':') : [];
  if (left.some(x => !x || !/^[0-9a-f]{1,4}$/.test(x)) || right.some(x => !x || !/^[0-9a-f]{1,4}$/.test(x))) return null;
  const missing = 8 - left.length - right.length; if ((parts.length === 1 && missing !== 0) || (parts.length === 2 && missing < 1)) return null;
...
function isPrivateIp(address) {
  if (net.isIPv4(address)) return isPrivateIpv4(address); if (!net.isIPv6(address)) return true;
  const mapped = address.toLowerCase().replace(/^::ffff:/, ''); if (net.isIPv4(mapped)) return isPrivateIpv4(mapped);
  const n = ipv6ToBigInt(address); if (n === null) return true; const top7 = n >> 121n; const top10 = n >> 118n; const top64 = n >> 64n;
...
async function resolvesToPublicDnsHost(hostname) {
  ...
  try { const answers = await lookup(hostname, { all: true, verbatim: true }); if (!answers.length) return false; return answers.every(answer => !isPrivateIp(answer.address)); } catch { return false; }
}
function safeProviderUrl(value) {
  try { const u = new URL(value); if (u.protocol !== 'https:') return false; const host = u.hostname.toLowerCase().replace(/[\[\]]/g, ''); if (net.isIP(host)) return false; if (host === 'localhost' || host.endsWith('.localhost') || host === 'metadata.google.internal' || host === 'metadata') return false; return true; } catch { return false; }
}
async function testConnection(baseUrl, credentialRef) {
  const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), TEST_TIMEOUT_MS);
  try { const headers = { Accept: 'application/json' }; if (credentialRef) headers['X-BAA-Credential-Ref'] = credentialRef; const response = await fetch(baseUrl, { method: 'GET', headers, signal: controller.signal, redirect: 'manual' }); const result = { ok: response.ok, httpStatus: response.status, redirected: response.status >= 300 && response.status < 400 }; try { await response.body?.cancel?.(); } catch (_) {} return result; }
  catch (error) { return { ok: false, httpStatus: null, timeout: error?.name === 'AbortError' }; }
  finally { clearTimeout(timer); }
}
export default async function handler(req, res) {
  noStore(res);
  try {
    const session = await requireAuth(req); if (!allowed(session)) return json(res, 403, { error: { code: 'FORBIDDEN', message: 'Teacher or administrator role required.' } });
    if (req.method === 'GET') { const rows = await sql`SELECT id, provider, base_url AS "baseUrl", status, last_sync_at AS "lastSyncAt", last_error AS "lastError", metadata, created_at AS "createdAt", updated_at AS "updatedAt" FROM erp_connections WHERE owner_user_id=${session.user_id} ORDER BY created_at DESC`; return json(res, 200, { ok: true, connections: rows.rows }); }
    if (req.method === 'POST') {
      const b = body(req); const action = clean(b.action || req.query?.action);
      if (action === 'configure') {
        const provider = clean(b.provider).toLowerCase(); const baseUrl = clean(b.baseUrl, 1000);
        ...
        const connectionId = clean(b.id) || id('erp'); const credentialRef = clean(b.credentialRef, 240) || null; const metadata = boundedMetadata(b.metadata);
      ...
      if (action === 'test') {
        const connectionId = clean(b.connectionId || req.query?.id); if (!connectionId) return json(res, 400, { error: { code: 'ERP_CONNECTION_ID_REQUIRED', message: 'connectionId/id is required.' } });
        ...
        const result = await testConnection(connection.rows[0].baseUrl, connection.rows[0].credentialRef); const status = result.ok ? 'configured' : 'error';
      ...
      if (action === 'sync') {
        const connectionId = clean(b.connectionId || req.query?.id); const direction = clean(b.direction || 'pull').toLowerCase(); const entityType = clean(b.entityType || 'students').toLowerCase();
        ...
        if (!connection.rows.length) return json(res, 404, { error: { code: 'ERP_CONNECTION_NOT_FOUND', message: 'ERP connection not found.' } });
        const runId = id('erpsync'); await sql`INSERT INTO erp_sync_runs(id,connection_id,direction,entity_type,status,error_message) VALUES(${runId},${connectionId},${direction},${entityType},'failed','EXTERNAL_PROVIDER_REQUIRED: vendor credentials and provider adapter are required for live ERP synchronization.')`;
  ...
  } catch (e) { return json(res, e.status || 500, { error: { code: e.code || 'ERP_FAILED', message: e.status ? e.message : 'Unable to process ERP integration.' } }); }
}
