...
export default async function handler(req,res){
 ...
 try{
  ...
  return json(res,200,{ok:true,learnerId,filters:{subject:subject||null,chapter:chapter||null},outcomes,evidenceRule:{minimumComparableObservations:2,singleObservationsAreNotCharacterized:true},limitation:'Outcome change is reported only from comparable submitted/evaluated assessment observations; activity alone is not treated as learning.'},noStore);
 }catch(e){return json(res,e.status||500,{error:{code:e.code||'OUTCOMES_FAILED',message:e.status?e.message:'Unable to load learning outcomes.'}},noStore);}
}
