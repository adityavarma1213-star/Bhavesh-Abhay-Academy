...
// These three helpers (corsHeaders/jsonError/jsonResponse) and getClientIp
// below were missing from this file — every call site already assumed them
// (same contract as api/chat.js), so every request here was throwing
// ReferenceError before reaching Gemini. Restored with the exact signatures
...
export default async function handler(req) {
  if (req.method === 'OPTIONS') return new Response(null, { status: 204, headers: { 'Cache-Control': 'no-store', ...corsHeaders(req) } });
  ...
  try { session = await requireAuth(req); } catch (e) {
    return jsonError(req, e.status || 401, 'AUTH_REQUIRED', e.message || 'Authentication required.');
  }
  let rate;
  try { rate = await consumeAiRateLimit('ai-mode', session.user_id || getClientIp(req), { windowSeconds: 300, maxRequests: 12 }); }
  catch { return jsonError(req, 503, 'RATE_LIMIT_UNAVAILABLE', 'AI rate-limit service is temporarily unavailable.'); }
  if (rate.limited) return jsonError(req, 429, 'RATE_LIMITED', 'Too many AI Mode requests. Please wait a moment.');
