...
async function upcomingForecasts(learnerId) {
  const catalog = await sql`
    ...
    WHERE NOT EXISTS (SELECT 1 FROM assessment_attempts aa WHERE aa.learner_id=${learnerId} AND aa.assessment_id=a.id AND aa.status IN ('submitted','evaluated'))
  ...
  const questions = await sql`
    ...
    LEFT JOIN learning_memory lm ON lm.learner_id=${learnerId} AND lm.concept=q.concept
  ...
  return catalog.rows.map(assessment => {
    ...
    const band = confidenceBand(known.reduce((sum, row) => sum + Number(row.evidence_count || 0), 0), new Set(known.map(row => row.concept)).size);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const attempts = await sql`
      ...
      WHERE learner_id=${learnerId} AND status IN ('submitted','evaluated') AND score IS NOT NULL AND max_score > 0
      ORDER BY COALESCE(end_time, start_time) DESC LIMIT 20`;
    const memory = await sql`
      SELECT concept, status, evidence_count, correct_count FROM learning_memory
    ...
    if (attempts.rows.length < 2 || conceptRows.length < 2 || band === 'insufficient_evidence') return json(res, 200, {
      ...
      evidence: { assessments: attempts.rows.length, trackedConcepts: allConceptRows.length, evidenceBackedConcepts: conceptRows.length, rawEvidence: evidenceCount, minimumConceptEvidence: MIN_CONCEPT_EVIDENCE },
    ...
    return json(res, 200, {
      ...
      evidence: { assessments: attempts.rows.length, trackedConcepts: allConceptRows.length, evidenceBackedConcepts: conceptRows.length, mastered, needsRevision, rawEvidence: evidenceCount, minimumConceptEvidence: MIN_CONCEPT_EVIDENCE },
      upcomingForecasts: upcoming, scope: 'academic_forecast_only',
