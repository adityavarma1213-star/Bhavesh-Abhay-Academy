...
// mirrors /api/speak but rate-limits by IP and does not require a session.
...
export default async function handler(req){
 ...
 const payload={contents:[{parts:[{text:text.slice(0,MAX_TEXT_CHARS)}]}],generationConfig:{responseModalities:['AUDIO'],speechConfig:{voiceConfig:{prebuiltVoiceConfig:{voiceName:voice}}}}};
 ...
 const inline=data?.candidates?.[0]?.content?.parts?.[0]?.inlineData||data?.candidates?.[0]?.content?.parts?.[0]?.inline_data; if(!inline?.data)return err(req,502,"The AI didn't return any audio — try again.");
 return new Response(pcmToWav(base64ToBytes(inline.data)),{status:200,headers:{'Content-Type':'audio/wav','Cache-Control':'no-store',...cors(req)}});
}
