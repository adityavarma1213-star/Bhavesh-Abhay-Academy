...
function validateBody(body) {
  const { text, subjectHint, imageAttached, imageDataUrl, submissionId } = body || {};
  ...
  if (imageDataUrl !== undefined && imageDataUrl !== null) {
    if (typeof imageDataUrl !== 'string' || imageDataUrl.length > 7000000) return { error: 'imageDataUrl is invalid or too large' };
    const m = imageDataUrl.match(/^data:(image\/(?:png|jpeg|webp));base64,([A-Za-z0-9+/=]+)$/);
    if (!m) return { error: 'imageDataUrl must be a supported base64 image data URL' };
    cleanImageDataUrl = { mimeType: m[1], data: m[2] };
  }
  if (imageAttached === true && !cleanImageDataUrl) return { error: 'imageDataUrl is required when imageAttached is true' };
  return { text: textResult.text, subjectHint: cleanSubjectHint, imageAttached: imageAttached === true, imageData: cleanImageDataUrl };
}
