...
export default async function handler(req,res){try{const s=await requireAuth(req);const learnerId=String(req.query?.learnerId||'');await requireLearnerAccess(s,learnerId);if(req.method==='GET')return json(res,200,{ok:true,submissions:await snapshot(learnerId)});if(req.method!=='PUT')return json(res,405,{error:{code:'METHOD_NOT_ALLOWED',message:'GET or PUT required.'}},{Allow:'GET, PUT'});const b=req.body||{};const rows=Array.isArray(b.submissions)?b.submissions:[];for(const x of rows){
...
}await writeAudit({actorUserId:s.user_id,action:'homework.sync',entityType:'learner',entityId:learnerId,metadata:{submissions:rows.length}});return json(res,200,{ok:true,submissions:await snapshot(learnerId)});}catch(e){return json(res,e.status||500,{error:{code:e.code||'HOMEWORK_SYNC_FAILED',message:e.status?e.message:'Homework sync failed.'}});}}
