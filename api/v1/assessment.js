...
export default async function handler(req,res){
  try{
    ...
    await writeAudit({actorUserId:s.user_id,action:'assessment.sync',entityType:'learner',entityId:learnerId,metadata:{attempts:attempts.length,evidence:evidence.length}});
    return json(res,200,{ok:true,snapshot:await snapshot(learnerId)});
  }catch(e){return json(res,e.status||500,{error:{code:e.code||'ASSESSMENT_SYNC_FAILED',message:e.status?e.message:'Assessment sync failed.'}});}
}
