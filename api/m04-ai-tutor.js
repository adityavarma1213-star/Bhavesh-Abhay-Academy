...
async function buildEvidenceContext(learnerId){
  ...
  const [memory,evidence,attempts]=await Promise.all([
    sql`SELECT concept,status,evidence_count,correct_count FROM learning_memory WHERE learner_id=${learnerId} AND status IN ('mastered','learning','needs_revision') AND evidence_count>=${MIN_EVIDENCE} ORDER BY last_updated DESC LIMIT 24`,
    sql`SELECT concept,subject,chapter,error_type,correctness FROM learning_evidence WHERE learner_id=${learnerId} ORDER BY created_at DESC LIMIT 80`,
    sql`SELECT score,max_score FROM assessment_attempts WHERE learner_id=${learnerId} AND status IN ('submitted','evaluated','completed') AND score IS NOT NULL AND max_score>0 ORDER BY COALESCE(end_time,start_time) DESC LIMIT 8`
  ]);
  const states=memory.rows.map(row=>({concept:clean(row.concept,70),status:clean(row.status,30),evidence:Number(row.evidence_count||0),correct:Number(row.correct_count||0)})).filter(row=>row.concept&&row.evidence>=MIN_EVIDENCE);
  const evidenceCounts=new Map();
  for(const row of evidence.rows){const concept=clean(row.concept,70);if(concept)evidenceCounts.set(concept,(evidenceCounts.get(concept)||0)+1);}
  const mistakes=evidence.rows.filter(row=>row.correctness!==true&&row.correctness!=='correct').filter(row=>{const concept=clean(row.concept,70);return concept&&Number(evidenceCounts.get(concept)||0)>=MIN_EVIDENCE;}).slice(0,8).map(row=>[clean(row.subject,30),clean(row.chapter,40),clean(row.concept,60),clean(row.error_type,40)].filter(Boolean).join(' / ')).filter(Boolean);
