...
export default async function handler(req, res) {
  ...
  try {
    ...
    // The account is already gone, so this audit record is intentionally
    // best-effort and is only written if the configured database still
    // permits a non-user actor record. Do not block deletion on audit output.
    try {
      await writeAudit({
        actorUserId: null,
      ...
      // Deletion has already committed; never recreate or partially restore
      // an account just because an audit sink is unavailable.
    }
