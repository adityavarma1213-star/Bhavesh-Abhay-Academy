...
// receives image bytes only for the transient vision evaluation request and never persists them.
...
function validateBody(body) {
  const { text, subjectHint, imageAttached, imageDataUrl, imageDataUrls, submissionId } = body || {};
  ...
  const cleanImages=[];
  const candidates=[];
  if(imageDataUrl!==undefined&&imageDataUrl!==null)candidates.push(imageDataUrl);
  if(Array.isArray(imageDataUrls))candidates.push(...imageDataUrls.slice(0,3));
  ...
  return {text:textResult.text,subjectHint:cleanSubjectHint,imageAttached:imageAttached===true,imageData:cleanImages};
}
...
function buildPrompt(text, subjectHint, imageAttached) {
  return (
    ...
    (imageAttached
      ? `NOTE: the student also attached a photo. Inspect the image itself. Read only what is actually visible; if handwriting/diagram content is unreadable, say so and request human review rather than guessing.\n\n`
      : '') +
    ...
    `  "imageEvaluated": ${imageAttached ? "true" : "false"},\n` +
    ...
    `- schemaVersion must be exactly 1 and evaluationType must be exactly "image_or_text".\n` +
    `- imageEvaluated is true only when an actual image was supplied and you genuinely inspected it; otherwise false.\n` +
...
const EVALUATION_TYPE = 'image_or_text';
...
function buildHumanReviewReasons(parsed, imageAttached) {
  ...
  if (imageAttached && parsed?.imageEvaluated !== true) reasons.push('The attached photo could not be confidently evaluated.');
...
export default async function handler(req) {
  ...
  let rate;
  try { rate = await consumeAiRateLimit('evaluate-homework', session.user_id || getClientIp(req), { windowSeconds: 300, maxRequests: 20 }); }
  ...
  const { text, subjectHint, imageAttached, imageData } = validated;

  const parts = [{ text: buildPrompt(text, subjectHint, imageAttached) }];
  for(const image of (imageData||[])) parts.push({ inlineData: { mimeType: image.mimeType, data: image.data } });
  const payload = {
  ...
  const result = {
    ...
    humanReviewReasons: [],
    imageEvaluated: imageAttached === true && parsed.imageEvaluated === true,
  ...
  // Server-authoritative verdict: bound to this exact submission id and the exact text
  // that was evaluated, so persistence (api/v1/homework.js) never has to trust a client's
  // self-reported overallAssessment/learningSignals for anything that feeds Learning Memory.
  const textHash = hashHomeworkText(text);
  const verdictToken = submissionId
    ? issueHomeworkVerdict({
        submissionId, textHash, overallAssessment: result.overallAssessment,
