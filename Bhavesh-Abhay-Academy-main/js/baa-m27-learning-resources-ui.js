...
function safeHttpsUrl(value){
  ...
  try{
    const parsed=new URL(trimmed,global.location?.origin||undefined);
...
async function load(format){
  ...
  status.textContent='Loading server-backed recommendations…'; list.replaceChildren();
  ...
  result.recommendations.forEach(item=>{
    ...
    article.style.cssText='padding:14px 0;border-top:1px solid rgba(253,249,240,.08)';
    const title=document.createElement('div');
    title.style.fontWeight='800';
    title.textContent=String(item.title||item.concept||'Learning resource');
    const meta=document.createElement('div');
    meta.style.cssText='font-size:.76rem;color:rgba(253,249,240,.55);margin-top:4px';
    const concept=String(item.concept||'');
    const reason=String(item.reason||'');
    meta.textContent=concept+(reason?' · '+reason:'');
    ...
    if(url){
      ...
      link.style.cssText='display:inline-block;margin-top:8px;color:#4CD9E8;font-size:.8rem;font-weight:700';
      link.textContent='Explore external search →';
