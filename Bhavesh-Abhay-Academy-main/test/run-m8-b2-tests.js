...
async function testStructuredSchemaAccepted() {
  const BAAHomework=freshHomework(makeLocalStorage(),()=>jsonResponse(200,{schemaVersion:1,evaluationType:'image_or_text',overallAssessment:'good',summary:'Clear work.',strengths:['Correct method'],mistakes:[],suggestions:['Check units'],confidence:'high',humanReviewRequired:false,humanReviewReasons:[],imageEvaluated:false}));
  ...
  assert(res.submission.evaluation.schemaVersion===1 && res.submission.evaluation.evaluationType==='image_or_text','B2-2: schema version and evaluation type are recorded');
...
async function testImageForcesHumanReview() {
  const BAAHomework=freshHomework(makeLocalStorage(),()=>jsonResponse(200,{schemaVersion:1,evaluationType:'image_or_text',overallAssessment:'good',summary:'Text is clear.',strengths:[],mistakes:[],suggestions:[],confidence:'high',humanReviewRequired:false,humanReviewReasons:[],imageEvaluated:false}));
  ...
  assert(res.submission.evaluation.humanReviewRequired===true,'B2-7: an attached image that was not evaluated requires human review');
}
function testEndpointSchemaContractPresent() {
  ...
  assert(src.includes("EVALUATION_TYPE = 'image_or_text'"),'B2-9: endpoint defines an explicit evaluation type');
  assert(src.includes('humanReviewReasons'),'B2-10: endpoint exposes human-review reasons');
  assert(src.includes('imageDataUrl'),'B2-11: endpoint accepts transient image data');
  assert(src.includes('inlineData'),'B2-12: endpoint sends supplied image bytes to Gemini as inlineData');
  assert(src.includes('confidence'),'B2-13: endpoint validates confidence');
}
